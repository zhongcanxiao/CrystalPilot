...
class mantid_workflow(BaseModel):
    """
    A model class.
    ...
    #while True:
        ...
        try:
            ...
            current_run_end_time = mtdapi.mtd['live_event_ws'].getRun().endTime().totalNanoseconds() * 1e-9  # Convert nanoseconds to seconds

            measure_time = current_run_end_time -self.initial_run_start_time
        ...
        measure_time = current_run_end_time -current_run_start_time
        ...
        mtdapi.SaveIsawPeaks(Inputworkspace='live_predict_peaks_ws', 
            Filename= self.output_path + live_peaks_fname)
        mtdapi.SaveIsawUB(Inputworkspace='live_predict_peaks_ws',  
            Filename= self.output_path + live_peaks_ub_fname)
        ...
        if intensity_ratio is not None and Rsig is not None and proton_charge is not None:
            self.proton_charges.append(proton_charge)
            self.intensity_ratios.append(intensity_ratio)
            self.rsigs.append(Rsig)
            self.measure_times.append(measure_time)  # Only append if all other values exist
        else:
        ...
        results = np.column_stack((self.measure_times, self.proton_charges, self.intensity_ratios, self.rsigs))
        np.savetxt(self.output_path + 'live_data_%s_results.csv'%(str(current_run)), results, delimiter=',', header='', comments='')
