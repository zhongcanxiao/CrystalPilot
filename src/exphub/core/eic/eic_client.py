...
# The comm classes, exception hierarchy, and deployment constants were
# extracted into sibling modules (auth/transport/errors/constants); they are
# re-exported here so this module's public surface is unchanged. EICClient
# below still constructs both comm classes. `requests`/`requests_oauthlib`
# stay imported here: the characterization tests monkeypatch their module
# attributes through this namespace.
from .auth import EICClientAuthComm
from .constants import (
    default_eic_ssl_port,
    default_ping_fed_host_url,
    default_system_openssl_path,
    default_url_base_dev,
    default_verify_ssl,
...
from .transport import EICClientComm
...
class EICClient(object):
    ...
    def __init__(
        self,
        eic_token: str,
        ipts_number: Any = None,
        beamline: Any = None,
        url_base: Any = None,
        ping_fed_host_url: Any = None,
        system_openssl_path: Any = None,
        verify_ssl: Any = None,
        eic_ssl_port: Any = None,
    ) -> None:
        """
    ...
    def _deserialize_outer_data(self, handle_exceptions: bool = True) -> Tuple[Any, Any]:
        """
        ...
        try:
            ...
            if handle_exceptions:
                first_char = (
                    outer_id_plaintext[0] if (outer_id_plaintext is not None and (len(outer_id_plaintext) > 0)) else ""
                )
                log_message = (
                    f"Exception in EICClient._deserialize_outer_data() serialized data: [{outer_id_plaintext.decode()}]"
    ...
    @staticmethod
    def _get_serialized_data_as_string(serialized_data: Any) -> str:
        """
        Converts serialized data to a string (TODO: why is this necessary?????).
        ...
        if (
            ...
            and serialized_data[0] == "b"
            and serialized_data[1] == "'"
            and serialized_data[-1] == "'"
        ):
            ser_str = ast.literal_eval(serialized_data)
        else:
            ser_str = serialized_data
        return ser_str

    @staticmethod
    def _log_exception(log_message: str) -> Optional[BaseException]:
        """
        Log exception.
    ...
    def http_auth_call_base(self, relative_url, submit_func, data=None, print_traceback_on_exception=False):
        ...
        try:
            ...
            ssl_cert_verify_failed, may_need_pip_system_certs = self._check_error_message_for_troubleshooting_clues(
                error_message
            )
    ...
    def http_auth_call(self, uri, data, http_method, do_auth=True, scopes=None, print_traceback_on_exception=False):
        """
        Make HTTP call
    ...
    # noinspection GrazieInspection
    def call_eic_ping_base(
        self, uri, do_auth=True, scopes=None, print_traceback_on_exception=False, propagate_exceptions=False
    ):
        """
        ...
        try:
            ...
            raw_response, ssl_cert_verify_failed, may_need_pip_system_certs = self.http_auth_call(
                uri, data, "get", do_auth=do_auth, scopes=scopes, print_traceback_on_exception=False
            )
    ...
    def call_eic_ping_authenticated_read_scope(self, scopes=None, print_results=False, propagate_exceptions=False):
        """
        ...
        response_data = self.call_eic_ping_base(
            "/eic/ping_authenticated_read_scope",
            scopes=scopes,
            print_traceback_on_exception=print_results,
            propagate_exceptions=propagate_exceptions,
        )
    ...
    def call_eic_ping_authenticated_write_scope(self, scopes=None, print_results=False, propagate_exceptions=False):
        """
        ...
        if scopes is None:
            scopes = self.write_scope
        response_data = self.call_eic_ping_base(
            "/eic/ping_authenticated_write_scope",
            scopes=scopes,
            print_traceback_on_exception=print_results,
            propagate_exceptions=propagate_exceptions,
        )
    ...
    def submit_table_scan(self, parms=None, desc=None, run_mode=None, headers=None, rows=None, simulate_only=None):
        """
        Submit Table Scan 1.
    ...
    def is_eic_enabled(self, print_results=False, disabled_on_exception=True, print_traceback_on_exception=False):
        """
        ...
        try:
            ...
            success, response_data = self.call_eic_standard(
                eic_command, eic_parms, print_traceback_on_exception=print_traceback_on_exception
            )
            if not isinstance(response_data, dict):
                raise Exception(
                    f"EIC response data is not a dict:"
            ...
            if (success is not None) and success and (is_enabled is not None):
                ...
                raise Exception("EIC call to get EIC status had invalid response.")
        except:
            e = sys.exc_info()
